...
- Три узды, каждая краснеет на том дефекте, ради которого заведена, — для трёх классов, повторившихся трижды и больше (`tests/test_review.py`): `GateRegistryTest` держит живой таблицу «ворота → тест» — новые ворота `check` без записанного теста роняют прогон; `SourceRuleTest` держит правила, что вызов git со списком путей идёт с `-z`, что распознавание ограды живёт в одном месте и что каждая числовая константа несёт над собой источник; `HandWrittenInputTest.test_ни_одна_команда_не_роняет_трейсбек_на_битом_определении` берёт список подкоманд у самого инструмента, так что новая команда попадает под правило без правки теста.

### Ломающие изменения

- **Вердикт внутри ограды кода больше не закрывает гипотезу, а нумерованная таблица вне раздела гипотез больше не таблица вердиктов.** У идущего ревью, чей отчёт поместил вердикты внутрь блока ``` или в таблицу `| 1 | … | проверена |` под другим заголовком, эти гипотезы станут без вердикта. Перенесите вердикты в обычные строки отчёта (шаблоны ролей теперь об этом говорят) или поставьте таблицу под заголовок «Гипотезы» и перезапустите `check`.
