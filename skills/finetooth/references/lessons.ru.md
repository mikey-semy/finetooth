...
13. **Отложенная находка переживает всё ревью незамеченной**, потому что не считается
    открытой. → У `deferred` обязательна причина, и именно причина превращает отсрочку в
    принятый риск: сводка публикует каждую отложенную находку в разделе «Принятые риски» с
    той причиной, которую кто-то назвал. Отложить — это решение, которое уезжает из ревью,
    а не долг починки к его концу; запрещено другое — откладывать молча.
