...
1. **Дифф вклеен ниже целиком — читай его, а не отчёт исполнителя.** Отчёт — заявление,
   ...
   себе. Объём диффа назван прямо над ним: если он не помещается в то, что ты держишь за
   раз, — **скажи об этом** и попроси более узкий диапазон (`--diff <часть>`): только это
   и уменьшает дифф, а `--scope` делит не вклеенное, а ответственность за отчёт. Прочитать
   сколько влезло и отчитаться за целое — ровно то, ради чего это правило и написано.
...
11. **Скажи прямо, нужен ли следующий круг.** Мерило — не число находок, а что в проекте
    ...
    отдельно: так машина начинает работать на себя. **Следующий круг нужен только при
    находке уровня medium и выше** (неверное поведение, тест, который не краснеет без
    правки, правка не по всем адресам дефекта, раздутая правка); low чинит исполнитель или
    ведущая без нового круга. Скажи, сколько твоих находок внутри кода, который написал дифф
    этого круга. **Остановка механическая:** если твоя главная находка — medium и выше —
    лежит на строке, которую написал этот дифф, инструмент отказывает в следующем круге
    починки, а `check` предупреждает, пока человек не запишет решение (`decide`); круг,
    который чинит собственную прошлую починку, — петля, а не движение. Инструмент читает
    `file` и `line` находки, поэтому давай каждой точную строку в коде после этого диффа.
12. **Пиши отчёт по ходу.** Установленное сразу клади в `{{REPORT_PATH}}` и дополняй;
    прогон, оборванный на лимите, сохранит то, что на диске, и потеряет то, что было
    только в голове.
13. **Коммиты следуют правилам коммитов проекта.** Если у проекта есть своя проверка
    коммитов (её называют джобы CI и CONTRIBUTING; например `.github/dco.sh <диапазон>`),
    прогони её по диапазону диффа и вклей вывод в отчёт. Коммит, который она отвергает, —
    без подписи, не под той идентичностью, с сообщением против соглашения проекта, — находка:
    CI проекта из-за него отвергнет правку. Что инструмент нашёл в файлах проекта:
...
Решение — это задание, по которому работал исполнитель, а не пожелание: суди правки по
нему, и правка, которая его обходит, — находка.
...
фразой, одинаково для всех экземпляров. В реестр он уходит полем `root` находки; с
третьего экземпляра проверка состояния требует закрыть класс уздой.
Узда записывается на находку, а не на корень: проверь по `roots`, что каждая записанная
исполнителем узда краснеет на дефекте каждой находки, на которую записана, — узда, зелёная на
одной из них, — находка.
...
Да/нет и почему — по мерилу из правила 11. Из находок выше: N — в коде, который написал дифф
этого круга, M — вне его, и лежит ли там главная. Если да — следующий ход за решением
человека, и инструмент его спросит.
```
...
Подтверждённые находки ведущая сессия заносит в реестр добором, даже уже починенные:
реестр — память ревью. Черновик — в `docs/review/reports/{{BLOCK_ID}}-findings.jsonl`, затем
`import {{BLOCK_ID}} --append --round {{ROUND}} --diff {{DIFF_PINNED}}` — два флага
записывают, какая проверка нашла строки и какой дифф она читала (`found_in`, диапазон
закреплён номерами коммитов); это и читает сигнал петли.
