...
`run-role.sh`, `guard-grep.sh`): 45 findings over the hunt and four rounds of fix review, all confirmed by execution — 42 fixed, 1 deferred and 2 rejected, each with its reason in the register.
...
- **Fences: one anchored rule instead of column arithmetic.** A fence opens at any indentation and closes only within three spaces of the column it opened at; a fence that never closes is read as text. The previous rules let an example's closing fence open a new one and swallow a manifest (four hypotheses became one, `check` green), then let an indented fence inside a fence close it and leak its verdicts. Held by a two-sided table, `QuotationMapTest` (document → quoted map), red under each of the three earlier variants. `run-role.sh` exits 3 when the run succeeded but the journal line was lost, instead of hiding it behind 0.
