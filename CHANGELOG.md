...
- Проверка, пройденная на 0.2.0, может покраснеть: блоки после проверки и открытые находки
  без отпечатков теперь отказ. Что делать: `backfill` один раз после обновления. Шапка
  `coverage.tsv` с коммитом читается по-прежнему; перезапишется при следующем `coverage`.
