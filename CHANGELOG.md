...
`run-role.sh`, `guard-grep.sh`): 51 findings over the hunt and five rounds of fix review, all confirmed by execution — 49 fixed, 1 deferred and 1 rejected, each with its reason in the register.
...
- **Quotation, decided by one principle: in doubt, the gate goes red.** A fence opens at any indentation and closes only near the column it opened at. A fence that never closes is read as text in what DEFINES the work (a manifest: more hypotheses to answer) and as a quotation in what REPORTS it (a report: fewer verdicts counted) — both directions stricter. In a report, a verdict inside an inline code span is a quotation of the form, except the list item that opens with a span holding the hypothesis id — the answer form the template used to prescribe (the template now asks for plain text). A report with a hypotheses section answers there; a hypothesis id mentioned elsewhere is not a verdict. Measured on every live register (two projects, 14 blocks): not one verdict moved. Held by `QuotationMapTest`, red under each earlier variant of the rule. `import` refuses a draft row over the claim/scenario limits `check` holds. `run-role.sh` exits 3 when the run succeeded but the journal line was lost.
