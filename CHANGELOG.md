...
- **`ROLE_DENY` in `run-role.sh` — the one limit the runner has** (#27). An optional environment variable passed to `claude -p` as `--disallowedTools "$ROLE_DENY"`, for runs that must not reach something (the repository's history, a neighbouring repository); unset, the command line is what it was. The script's header gives the example `ROLE_DENY="Bash(git *),Read(//home/me/other-repo/**)"` and says why the `//`: measured, a deny rule with `Read(/home/x/**)` matched nothing and the read went through, while `Read(//home/x/**)` refused Read, Grep and `cat` alike. The suite's `claude` stub now records its arguments. Mutations: the flag line removed → `test_role_deny_передаётся_клиенту_как_disallowedTools` red; the value passed unquoted (split into words) → the same red; the flag always passed, empty without the variable → `test_без_role_deny_запрета_нет` red.
- **The loop signal: a fix round whose top finding lies in the previous round's own code stops for a human decision** (#9). The kit's own review ran T2–T4 through two and three fix rounds each; from round 2 on almost every fix-review finding sat in the code the previous round wrote, mostly in the guard it had added, and only an owner's decision ended it — a decision the kit had nowhere to put. Three parts:
  - **`found_in` on a finding.** `import <ID> --append --round N --diff <range>` records which fix review found the new rows and the diff it read, `{"role": "fixreview", "round": N, "diff": "<base>..<tip>"}` — the range pinned to commit ids, because `main...HEAD` is another diff once the next round commits. The fix reviewer's prompt names that exact command (new substitution `{{DIFF_PINNED}}`), so the field is written by the tool, not by a lead appending "(fix review round N)" to the claim. Records without the field take no part and break nothing.
  - **The signal.** `prompt <ID> --role fix --round N` (N ≥ 2) refuses when the top open finding of fix review N−1 is medium or higher and lies on a line fix round N−1 wrote — by the hunk lines of `git diff -U0 <range> -- <file>`, not by file. `check` says the same as a warning (`loop/top-finding-in-own-diff`) for every block not closed.
  - **`review decide <ID> "<decision>"`** writes the human's decision to `docs/review/decisions.jsonl` (block, round, date, text; a file of its own because `state.json` is "where we are now" and `init --force` rewrites it) and a line to the journal. A decision recorded after review N−1 lifts the refusal, and every decision of the block reaches the fix and fix-review prompts through `{{DECISIONS}}` (a neutral line when there are none). The stop rule in the fix reviewer's template is rewritten as the mechanical one.

  Mutations, each against `LoopSignalTest`: the line test replaced by "the file is in the diff" → `test_находка_вне_строк_диффа_того_же_файла_круг_не_останавливает` red; the medium threshold removed → `test_сигнал_смотрит_на_главную_открытую_находку_от_medium` red; any medium+ finding instead of the top one → the same red; deferred findings counted → the same red; any decision lifts regardless of its round → `test_решение_человека_снимает_сигнал_и_доходит_до_заданий` red; no decision lifts → the same red; `{{DECISIONS}}` always neutral → the same red; the refusal in `prompt` removed → `test_главная_находка_внутри_диффа_прошлого_круга_останавливает_круг` red; `import` not writing `found_in` → four tests red; the range stored as given → `test_импорт_пишет_круг_и_дифф_закреплённый_номерами_коммитов` red; the import command in the fix reviewer's prompt unpinned → `test_ревьюер_правок_получает_команду_ввоза_с_закреплённым_диффом` red; a hand-written `found_in` of the wrong shape read as a dict → `test_старые_записи_без_found_in_не_роняют_и_не_останавливают` red (a traceback). The `check` warning is in the gate registry, and `GateMutationTest` silences it and flips it to a refusal.

- **`review sarif [--out FILE]` — the register's findings as SARIF 2.1.0 for GitHub code scanning**, so they show in the Security tab and on the lines of a pull request. Open and deferred findings are exported, closed ones (fixed, rejected, duplicate) are not. The rule is the finding's defect class (`root`), or its block when it names none; the level comes from the severity by a table (critical and high → `error`, medium → `warning`, low → `note`); the message is the claim, then the scenario, then the block report it is argued in (the verifier's, or the hunter's before there is one); the location is the path from the repository root, percent-encoded as an RFC 3986 reference, and the line (1 for a finding about a whole file); `partialFingerprints.primaryLocationLineHash` is the review's id and the finding's — the only key GitHub matches on, so an edited line does not open a second alert. A deferred finding carries a SARIF suppression (`external`, `accepted`, its reason as the justification) and also says "accepted risk" in its first sentence, because GitHub does not read suppressions. `security-severity` is not written: GitHub reads it on the rule, a rule here spans findings of several severities, and the register does not say which finding is a vulnerability. Printed to stdout; `--out` writes the one file it names — the second exception to "writes only under `docs/review/`", named in `SECURITY.md` and held by the write-boundary test. Checked against the OASIS specification and GitHub's "SARIF support for code scanning"; the suite checks the structure with its own validator (no network, no dependencies), and the validator is itself proven on sixteen kinds of damage (mutation: its URI check removed → the `сырой пробел в uri` subtest of `test_проверяющий_структуры_краснеет_на_каждом_нарушении` red). Mutations, each red on its test: closed statuses exported → `test_выгружены_открытая_и_отложенная_закрытые_нет`; high mapped to `warning` → `test_уровень_берётся_из_серьёзности_таблицей`; the rule always the block, and `ruleIndex` off by one → `test_правило_это_корень_а_без_корня_блок`; the scenario dropped, and the hunter's report preferred to the verifier's → `test_сообщение_это_утверждение_сценарий_и_отчёт_блока`; the fingerprint a hash of the line → `test_отпечаток_это_номер_находки_и_правка_строки_его_не_меняет`; the path not encoded → `test_путь_с_пробелом_и_кириллицей_относительный_и_закодирован`; line 0 for a whole-file finding → `test_находка_без_строки_указывает_на_начало_файла`; the suppression dropped, and the "accepted risk" prefix dropped → `test_отложенная_находка_принятый_риск_и_в_подавлении_и_в_тексте`; `--out` ignored → `test_out_пишет_файл_и_ничего_не_печатает_в_поток`; stdout mode also writing a file → `test_ни_одна_команда_не_пишет_вне_каталога_ревью`.
- **CI jobs to paste**: `assets/github-actions-snippet.yml` — `check` as a job to make a required status check, and the SARIF upload through `github/codeql-action/upload-sarif` pinned to the commit of v4.38.2 (`category: finetooth`, skipped for pull requests from forks, whose token cannot write alerts); `assets/gitlab-ci-snippet.yml` — `check` as a job with the full history (`GIT_DEPTH: "0"`; GitLab clones 20 commits deep by default). GitLab reads SARIF on the Ultimate tier only (GA since 19.2), so the GitLab file says so and leaves a Code Quality report as the next step. The rule that CI actions are pinned to a commit now covers the snippets too; mutation: the upload unpinned to `@v4` → `test_действия_ci_закреплены_коммитом` red.

### Changed

- **The role tool lists in `run-role.sh` are called what they are: pre-approvals, not limits** (#27). `--allowedTools` approves tools in advance on top of the operator's own permission settings and takes nothing away — measured: a run given `Read,Grep,Glob,Write` still called Bash. A reader could take the lists for a sandbox ("the hunter has no npm", "the verifier has no network"); the comment at the role table and `SECURITY.md` now say they are not, and that a limit is `--disallowedTools` (`ROLE_DENY`) or a sandbox. Mutation: the `SECURITY.md` paragraph removed → `test_обещание_безопасности_называет_то_же_исключение` red.
- **A finding's code fingerprint is the lines around it, not the whole file** (#37). In the kit's own review every PR that touched `review.py` or `tests/test_review.py` turned `check` red on the same 15 deferred findings — five PRs in a row, each time with the code under them untouched, each ending in a mass `restamp`. A finding with a line now stores `region_sha` (a hash of the line and `REGION_K` = 3 lines above and below it, trailing whitespace and line endings ignored, the window clipped at the file's edges) and `region_span` (how many lines above and below the window took). `check` looks for that window by content anywhere in the file: found — the finding is in place, wherever it now sits; not found — the refusal `finding/region-changed`, which also offers `restamp <ID> --line <N>` for a defect that now sits elsewhere. A finding without a line, or on a binary file or a symlink, keeps the whole-file `code_sha` as before. K is measured on the kit's own history, not chosen: of 148 fixed findings with a fix commit, the window disappears in the fix commit for 67 at K=0, 98 at K=3, 111 at K=10 (the whole file: 141); over 61 commits to the two hot files, a window around a line the commit left untouched breaks in 2.4% of cases at K=3 and 5.7% at K=10 (the whole file: every commit); from K=2 on no cited window repeats in its file. The price is named in the code: a fix more than three lines from the cited line is not seen by this gate, which is a backstop for a status nobody moved, not the way fixes are recorded.
  - **`restamp <ID>`** records the new line when the window only moved, and prints the line it anchored to, so a wrong anchor is seen at once. **`restamp <ID> --line <N>`** anchors the finding at another line. `import` carries a known finding's region together with its line, so a re-import does not bring back the line the hunter's draft cited before the shift.
  - **A register from an earlier version** keeps working: a record with `code_sha` and no `region_sha` is checked on the whole file exactly as before, and its refusal says that `restamp` moves it over. `restamp <ID>` on such a record takes the window from the version of the file its `code_sha` names (git still has the blob) and looks for it in the current file — so the line the finding cited is found where it is now, not taken as is; if the blob is gone or the code there changed, the recorded line stands and the printed anchor shows it. `backfill` stamps the region form on records with no fingerprint at all.
  - **The block fingerprint stays whole-file** (option 3 of #37 is not taken): a block claims to have read every line of its files, so a line added to one of them is code nobody reviewed, and the coverage map names files, not lines — there is no narrower thing a block vouches for.

  Mutations, each red on its test in `RegionFingerprintTest`: K 3→10 and K 3→1 → `test_правка_внутри_окна_роняет_проверку_за_его_краем_нет`; trailing whitespace not dropped, and leading whitespace dropped too → `test_пробелы_в_концах_строк_и_crlf_не_правка`; the window looked for only at the recorded line, the re-import dropping the carried line, and `restamp` keeping the old line → `test_правка_выше_находки_сдвигает_строку_но_не_роняет_проверку`; the first match instead of the nearest → `test_одинаковое_окно_ищется_ближайшее_к_записанной_строке`; `import` stamping the whole file → `test_импорт_снимает_отпечаток_окна_а_не_файла`; the legacy line taken as is, and the old `code_sha` kept beside the new fields → `test_старая_запись_проверяется_по_старому_а_restamp_переводит_её_на_окно`; the index fallback removed → `test_файл_из_индекса_не_выложенный_на_диск_находится`; a binary file given a region → `test_двоичный_файл_остаётся_на_отпечатке_файла`; no clipping at the top and at the bottom → `test_окно_у_краёв_файла_обрезается_и_находится`, `test_находка_на_последней_строке`; `backfill` stamping the whole file → `test_backfill_ставит_окно_находке_со_строкой`; `--line` ignored → `test_restamp_line_переносит_якорь_туда_где_дефект_теперь`. The new `check` gate is in the gate registry, and `GateMutationTest` silences it and flips its severity.

- **A finding is shown at the line its code sits on now, and `check` no longer warns that it moved.** With the region fingerprint (#37 above) a finding whose window only shifted stayed green, but `findings.md` and the SARIF export showed the line the register recorded, so `check` warned (`finding/line-moved`) until someone ran `restamp` — and in an actively edited file that was every PR, for a `restamp` that changed nothing but a display. Now every display of a finding — `findings.md`, `sarif`, the summary, the hunter's, verifier's and fixer's prompts, `roots` — looks the window up in the current file and shows the line it is on; not found, the recorded line, with the refusal `finding/region-changed` as before. The register is not rewritten: showing is not recording, and `restamp` is still the one command that writes a new line. `check` compares `findings.md` with the register as before, except the line of a region-fingerprinted finding, which only has to be a number there: the file rendered before a shift was right when it was rendered, and comparing that line would bring the same noise back as a refusal. **The warning is removed, not kept for a "large" shift**: the recorded line is read in one place only, to pick the nearest copy when the window repeats in its file, and in the measurement behind `REGION_K` no cited window repeats from K=2 on, so a drifting record costs nothing measured, and a threshold for "large" would be a number with no measurement behind it.

  Mutations, each red on its test in `RegionFingerprintTest`: the display returning the recorded line → `test_показ_берёт_строку_где_код_сейчас_реестр_не_тронут` and `test_одинаковое_окно_ищется_ближайшее_к_записанной_строке`; `check` comparing `findings.md` with the render in full, and the region findings never masked → the same two; the masked line accepting any text → `test_показ_берёт_строку_где_код_сейчас_реестр_не_тронут`; `findings.md`, `sarif`, the summary, the hunter's prompt, the fixer's prompt and `roots`, each reading the register's line → `test_показ_берёт_строку_где_код_сейчас_реестр_не_тронут`; a window not found showing no line instead of the recorded one → `test_окно_не_нашлось_показ_держит_записанную_строку`; the mask put on a record that has a window but no line, where the display writes none → `test_запись_с_окном_без_строки_не_роняет_сверку_findings_md`.

### Fixed

- **The freshness rule no longer sends a review of the past to read the future** (#27). In the recall experiment (the tree at the parent of a real fix) the hunter and the verifier followed the templates' freshness rule — `git log HEAD..origin/master`, `git show origin/master:<file>` — and read the fix itself; the runs had to be thrown away. The `hunter` and `verify` templates, in both languages, now say: when the block is reviewed at a fixed commit on purpose (a snapshot of the past, an archive, an experiment), do not compare with `origin`. Mutations: the sentence removed from `hunter.ru.md`, and from `verify.md` → `test_правило_свежести_не_сверяет_снимок_прошлого_с_origin` red.
...
The fix review of that block, round 1 — seven more findings about the round that closed
them, all closed here.

- **The fix reviewer's prompt carried the diff three times and measured it once.** `{{DIFF}}` was excluded from the one-pass substitution and applied afterwards as a plain replace over the **assembled** body, which by then holds the pasted block manifest — and a manifest that writes about the placeholder (this repository's own does, twice) had both mentions replaced with the whole diff. Measured on the kit itself: `{{DIFF_VOLUME}}` stated 464 KB, the prompt delivered 1410 KB. The diff is now a substitution like any other, made in the same pass, so what the manifest quotes stays a quotation; `{{DIFF}}` is no longer exempt from the unfilled-substitution refusal either. Mutations: the replace back to the assembled body → `test_дифф_вклеивается_один_раз_даже_если_манифест_о_нём_пишет` red; the exemption restored → `test_подстановка_диффа_в_чужой_роли_это_отказ_а_не_текст` red on the role that gets no diff, while the role that does still gets it whole.
- **The way out named beside the volume did not reduce the volume.** The measure told a reviewer who cannot hold the diff to take one half through `--scope` — and `--scope` renames the report and appends a line: measured, the prompt with it is 120 bytes **larger**, and its own sentence then says to read the rest for context. What shrinks the diff is a narrower `--diff` range, and that is what the measure now names, in both languages, saying plainly what `--scope` does instead; rule 1 of both fix-review templates, `SKILL.md`, the `--help` of the flag and `{{SCOPE_LINE}}` say the same. Guard: every string of the tool and every paragraph of the skill that names `--scope` must name `--diff` too — mutation: the measure and both templates restored → `test_везде_где_назван_scope_назван_и_флаг_уменьшающий_дифф` red at three addresses; the other side is that `--scope` still names the half in the report and in its file name.
- **The template contract was satisfied by prose.** `TemplateContractTest` asked only that each field name occur somewhere in some template — and every one of them is an ordinary word: `root` was "named" by the phrase "the whole root at once" and by the path `"file":"path/from/repository/root"`. Measured: every fenced draft schema deleted from all eight templates left the guard green on all thirteen fields in English. A field of the draft must now be named as a schema key (`"field":`) inside code, and a field the tool writes from a flag must be named in the same paragraph as the command that writes it. Mutations: all fences deleted → 16 subtests red; the English templates restored to their pre-fix state → red on `root` as well, the half that stayed green before.
- **Three negative tests asserted the gate's message, not its verdict.** A deferral without a reason, a rejection reason hidden in `claim` and the third instance of a class were each checked with `assertIn` over the whole output of `check`, which prints refusals and warnings into the same stream — so a gate moved from the refusals to the warnings kept them green while `check` exited 0 on the state it had refused. All three now read the refusals section. Mutation: each of the three gates moved to `warnings` in turn → its test red, the mutant building.
- **The verifier was handed the block whole with no reading budget.** `{{VOLUME}}` stood in the hunter's templates and nowhere else, while the verifier gets the same mechanically generated "read all" list and is the most expensive role of the four. Both verifier templates now carry the measure, and the over-the-ceiling warning names the coverage section of the reader's own report instead of the hunter's heading. Guard: a template that hands out the block's file list must state its volume — mutation: `{{VOLUME}}` removed from the two verifier templates → red on both files and on the prompt itself.
- **Two of the four texts about a deferred finding were held by nothing.** The lesson was guarded, the entry points were guarded; the refusal's own tail and the completion conditions in `SKILL.md` — the two that answer "is the review finished" — were not, and both restored to their pre-fix wording left the whole suite green. Now the refusal is read for what it says, and all seven texts that answer the question are held together. Mutations: the refusal restored → `test_отказ_ворот_говорит_то_же_что_сводка` red; `SKILL.md` restored → `test_все_тексты_набора_дают_один_ответ_об_отложенной` red on that file; the lessons restored → red on both of theirs.
- **A section's opening paragraph rendered inside the previous bullet.** The T2 paragraph of this file followed the last T4 list item with no blank line, which in CommonMark is a lazy continuation: the sentence was absorbed into the `NOTICE.md` entry and the section below it hung under no heading — in both languages, in the section release notes are taken from verbatim. Guard: no tracked markdown file may start a paragraph directly after a list item — mutation: either blank line removed → `test_ни_один_абзац_не_приклеен_к_предыдущему_пункту_списка` red, naming file and line.

The fix review of that block, round 2 — two more findings about the round that closed the
previous ones, both closed here. The third, a guard that `set-finding --rule` overwrote
across three blocks, is a decision about the tool and went to issue #28 as an accepted risk.

- **The over-the-ceiling warning named a section the gate does not read.** Fixing the verifier's missing budget, the warning was rewritten to send the unread rest into "the coverage section of your report" — and what `check` reads in a hunter report is the coverage-limits section: a hunter that followed the warning to the letter got "the 'Coverage limits' section of the hunter report is empty", in both languages, while the hunter's own template still pointed at the right one. The warning now names the section of the reader's OWN report that the gate reads for it — the hunter's coverage limits, the verifier's block coverage status, which is what the verifier's report actually has. Guard: every message of the tool and every paragraph of the skill that names an agent a report section, a flag or a command must name what the mechanism gives — the section readable by the gate over that role's report, a flag some entry point of the kit declares, a command the tool has. Mutations: the message re-pointed at the neighbouring section (English and Russian separately), at the other role's section, at a flag that does not exist and at a command that does not exist → `NamedExitTest` red on each, and red on the pre-fix code at four addresses; the other side is that rewording the same section in other words, naming a section of the invariants, a foreign program's flag or a real command all stay green. Behaviour: a report written exactly by the warning passes the coverage gate, while the same report with the unread named in the neighbouring section is still refused.
- **The verifier's half of that warning held its verdict by nothing.** Besides the section, the verifier's warning says what to write there — and only that part satisfies the coverage gate: the unread paths alone under "Block coverage status" are refused with "has no coverage verdict" (headings are not read). Deleting the clause left the whole suite green. The warning now quotes the words to open the section with ("Coverage is incomplete" / «Охват неполный»), and a test reads them out of the prompt in both languages, writes the verifier report by them and runs `check`. Mutations: the quoted words deleted → red on both languages; replaced with words the gate does not read → red on both.
- **The glued-paragraph rule refused correct CommonMark.** A heading, a thematic break or an HTML block directly after a list item ends the list and renders exactly as written — the rule flagged all three and told the author the markup makes them a continuation of the bullet. Lazy continuation belongs to a paragraph alone, and only a paragraph is flagged now; a table row is deliberately not exempt, because GFM builds a table header out of a paragraph's last line, so a row after a bullet really does stay inside it. The rule also lived inline in its test and was fed nothing but the repository's own files, which is why the false half was never noticed: it is a function now, with invented documents on both sides. Mutation: the rule back to flagging any line → `test_правило_про_склеенный_абзац_читается_на_выдуманном_документе` red on all three constructs while the repository-wide test stays green.
...
- **The class guards themselves were proved on the code as written, and only on it.** Twice running, the heaviest finding of this block was a guard keyed on incidental syntax, and each time the answer was one more spelling — after which the next spelling was found. The widened NUL rule folded `[…] + […]` but not a prefix held in a variable nor `[*PREFIX, …]`, and it never looked at an argv handed to a wrapper (`log_records`) rather than to `subprocess` at all; the gate registry read four ways of filling the refusal list but still only inside `cmd_check`, so a gate split out into a helper — the obvious next refactor — needed no entry and no test. A guard is proved now the way the gates are, by mutation of the REAL source. Every source rule reads a value where it is BUILT, through one resolver over the module's scopes, instead of where it is written: argv wherever it is assembled and whoever runs it, a gate wherever the refusal list is filled, a threshold inside the function that owns it, `--format=` however the string is put together, the message table however it is declared, a register record recognised by its own fields rather than by the name of the variable, a child process recognised through an import alias (and not recognised when a local name shadows it). `SourceMutationTest` runs all ten rules against the tool's source and the suite's source, respelled the way a refactor would write them and spoiled on purpose, and demands the SAME verdict from every spelling; a new source rule with no mutation table fails the suite. Two guards of the round before are named here for the first time: every source rule must live in its own function, and it must be fed an invented source besides the tool's own. Mutations: each of the ten rules put back to its previous shape in turn → the harness red for every one of them.
- **The suite's own refusal was unreadable in the locale it exists for.** The import-time refusal added in the round before explains in Russian why a non-UTF-8 locale cannot run this suite — and that is exactly the locale where `stderr` falls back to `backslashreplace`, so the explanation arrives as `ко…` and the operator has nothing but the ASCII command at its end. It is English ASCII now, the only such text in the suite. Test: the module is imported in a child under `LC_ALL=C PYTHONUTF8=0`, and the refusal must name the cause, name the command that lifts it and carry no escapes; the other side demands a silent import in a UTF-8 locale. Mutation: the Russian text back → escapes back, test red.
- **A class guard keyed on incidental syntax, closed with a single entry point instead of a longer list of spellings.** Three rounds running, the heaviest finding of this block was the same one: the NUL rule was taught one more way of assembling argv, the gate registry one more way of filling the refusals list — and each time the next spelling was found (argv grown with `.extend()`, argv returned by a `_git(*rest)` wrapper, a gate in a helper whose parameter is called `refusals`, all three measured with the whole suite green). Recognition is no longer the answer. The tool now starts a process in ONE place, `git()`: it adds `-z` to every run whose output carries paths and reads that output back (`.fields`, `.records`), so a call site cannot drop `-z` — it does not build the command line. `check` collects its refusals in ONE container, `Refusals`, and a refusal is a pair: the gate's own key, written at the call site, and the text a human reads; the registry therefore reads what a gate CALLS ITSELF instead of guessing from the shape of the line, and the exit code is the container's verdict and nothing else. `git log` is ordered and parsed in one function, so a second parser of the marked stream has nowhere to get one from. The guards shrank to three questions — is a process started outside `git()`, is a refusal added without a literal key, does `cmd_check` return anything but the container's verdict — and every enumeration of spellings they used to carry is gone. Mutations: git spawned outside the helper, argv accumulated with `.extend()`, argv handed over by a wrapper, the module under an alias, the spawner pulled in by name, a spawn through `os.popen` → the rule names the function in every spelling and ignores every shape of argv; a new gate in a helper whose container is called `refusals` → registered and demanded of the table; a refusal with a key assembled on the way → a refusal, not silence; a gate printing its own refusal and returning 1 → the exit rule red. `-z` itself is held by behaviour now: a tracked Cyrillic path must reach `coverage.tsv` as the index holds it, and removing the `-z` the helper adds turns seven tests of `GitTruthTest` red instead of none.
- **The guard over the guards recognised "a real source" by three marker strings**, so a rule fed the tool through a path expression (`(SKILL / "scripts" / "review.py").read_text()`) needed no mutation table at all. What a rule is fed no longer decides anything: a rule is a function that turns text into a tree and whose verdict reaches a test — through any chain of calls — and every one of them must be in `SOURCE_MUTATIONS`. Two rules the marker list had missed came under it: the exit rule above, and the reader of the tool's subcommand list, which the "every command is named in SKILL.md" gate rests on and which now takes a source instead of reading the file itself (and reads a command name where it is BUILT, so a name hoisted into a variable is still a command). Mutations: a rule reached through a relay, a rule written inline in a test, a subparser added without a line in `SKILL.md`, a command name moved into a variable → red where the marker table was silent.

The fix review of block T4, eight more findings, all closed here — five of them in the guards and the gate that block added, three in the documents, the register and the round's own report.

- **The sign-off gate turned away honest work and waved through work it never looked at.** The commit author's address went into a `grep -E` pattern unescaped, and an address is full of metacharacters: `+` is a quantifier, so a contributor committing under GitHub's privacy-protected address (`12345678+login@users.noreply.github.com` — the default for an account that keeps its email private, and what its web interface commits under) was refused as unsigned, and the refusal named `git rebase --signoff`, which regenerates the identical line: there was no way out of it at all. `.` is a wildcard, so a sign-off naming `aXb@example.com` certified a commit authored by `a.b@example.com` — the very thing the gate exists to refuse. And `git rev-list` was read through a process substitution, where `set -euo pipefail` never sees its exit code: on a range git cannot resolve — a clone without `origin/dev`, a remote named `upstream` — the gate examined no commits and printed "all commits in origin/dev..HEAD are signed off" with exit 0, which is the invocation CONTRIBUTING documents for a local run. Now the address is parsed out of the sign-off line and compared as a string, with case folded on both sides, and a range that does not resolve is a refusal naming the fetch that fixes it. Mutations: the old pattern restored → `test_подпись_с_адреса_с_метасимволами_проходит` and `test_подпись_на_похожий_адрес_не_засчитывается` red; the listing back behind a process substitution → `test_нерешаемый_диапазон_роняет_ворота_и_называет_починку` red; the other direction — a range that resolves to no commits is not a violation — is held by `test_диапазон_без_коммитов_ворота_не_роняет`.
- **Every refusal written in a shell script could be turned into a success with the suite green.** The mutation guard silences the gates inside `cmd_check` and cannot see a gate written as a script at all — and there are two such scripts. Besides the range check above, three refusals of `guard-grep.sh` were held by nothing: an unknown argument, no pattern, no paths, each of them what a Makefile reaches by a typo, and each the difference between "scanned nothing" and "clean tree". `ShellGateMutationTest` now turns each `exit N` of each shell gate into `exit 0` in a copy substituted through the environment and requires the suite named beside that script to go red; the three refusals got the tests they were missing. The refusal is found by shape, so one written tomorrow falls under the rule by itself. Mutations: all six refusals of the two scripts measured one at a time → the named suite red on every one; the other side — the substituted copy breaks nothing by itself — is `test_на_целой_копии_прогон_ворот_зелёный`.
- **"Every gate the documents declare is run by CI" was satisfied by two words anywhere in the workflow.** For `python3 -m unittest discover -s tests` the guard looked for `python3` — which occurs in the step that installs the skill validator — and for `tests`, which is the workflow's own name. Measured: deleting the step that runs the suite left it green, and so did deleting that step's whole job, while branch protection kept requiring a green `tests` workflow before a merge into `master`. A documented command is now matched against the body of a `run:` step: the same program by name (the validator lives in a venv under `$RUNNER_TEMP`, and a name survives a path), every argument of the documented command present and in order, a wider step allowed (`-v` on the suite), and a commit range matched by shape because CI takes it from the event payload. Mutations: the `Run the tests` step deleted, then its whole job deleted → `test_каждые_объявленные_ворота_гоняет_ci` red, where the old guard was green on both; the other direction — a step that spells the command its own way still counts — is held by `test_узда_видит_ворота_которых_ci_не_гоняет` on an invented workflow.
- **The write-boundary guard never reached three of the commands that write.** It called every subcommand with no arguments and with a block id, and `set-status`, `set-finding` and `log` are refused by argparse before their own code runs — so for three of the writing commands the promise in SECURITY.md was held by nothing, and a future `--out`-style flag on any of them would pass a guard described as one a new command falls under by itself. This is the blind spot the same series had already diagnosed and repaired one guard over; the fix went to one address of it and not the other. Both sweeps now take one table of the arguments at which a command reaches its body, and an argparse refusal no longer counts as a pass. The class is closed by a rule, not by two fixes: `CommandSweepRuleTest` reads the suite's own source and finds a sweep that takes the command list from the tool and never asks whether the command started — by shape, so the sweep nobody has written yet falls under it too. Mutations: the rule fed the suite as it stood → red, naming the boundary sweep; the other direction — a sweep that asks through the shared helper, and one carrying its own copy of the vocabulary — is held by `test_узда_видит_обход_которого_ещё_нет`.
- **The public scenario count was a band, not a measurement.** The guard was loosened to "no more than the real count and no less than nine tenths of it" so that a pull request adding a test would not go red, and the round then shipped with 270 in the documents against 296 in the suite: a reader sent to "Check before committing" sees one number and runs another, and cannot tell whether the checkout, the command or the document is wrong. The band is gone — the number in README (both languages) and AGENTS.md equals the measurement, and the refusal names the command to run and the three files to change. The measurement itself was taken with `unittest.defaultTestLoader`, which carries the patterns of `-k` inside it: under a filter the count collapsed, so the figure depended on how the suite was invoked. It has its own loader now. Mutations: a document left at the old count → `test_число_сценариев_в_документах_равно_настоящему` red; the shared loader back → `test_замер_числа_сценариев_не_зависит_от_способа_запуска` red.
- **A paragraph glued to a list item, in both CHANGELOGs.** The paragraph introducing block T2 followed the last bullet of block T4 with no blank line, and by markdown's lazy continuation it became part of that bullet: the release notes, which RELEASING gate 4 takes from this section verbatim, would have ended the `NOTICE.md` entry with "Block T2 of the same review — … 15 findings, all closed here." and put the fifteen bullets of that block under no heading at all. Both languages fixed, and held by a rule over every document in the root, taken from `git ls-files`: a paragraph may not start on the line after a list item. Mutation: the blank line removed again → `test_в_документах_нет_абзаца_приклеенного_к_пункту_списка` red; the other direction — an indented continuation, a nested item, a heading, a list inside a fence — is held by `test_узда_видит_абзац_приклеенный_к_пункту`.
- **Two defect classes were recorded as closed by guards that do not close them.** The register stores one guard per class, and for two classes it named a test that fires on one instance out of three and four: *a rule declared enforced with nothing enforcing it* was recorded against the test of the Python matrix — green with the whole `dco` job deleted from CI — and *a number in a public document not checked against its source* against the thresholds test, green with a README left at the old scenario count. Both measured by restoring the defect. The guard of each class is now the test that fires on its instances, and the finding that the tool cannot check this — nothing ties a rule to the instance it closes — is written down for the maintainer rather than papered over.
- **The round-1 fix report named a guard that does not exist.** It gave `test_число_сценариев_в_документах_равно_настоящему` as the guard of a finding, and a commit inside the same range had renamed the test and changed what it asserts; the report also quoted a test count from the fixer's own worktree rather than the merged tree. A reader checking the round against its report cannot tell a test that was dropped from one that was renamed or never written. Corrected by notes where the stale figures stand, not by a silent edit: the report is the record of that round.

The second fix review of block T4, nine more findings, eight closed here and one deferred to issue #28. Five of them are in the guards the previous round had just written: each was green on exactly the state it exists to forbid.

- **The write-boundary guard started three commands and reached the write of none.** The previous round gave every subcommand arguments at which it reaches its body — and on the guard's own stand `import` had no draft to read, the register held no finding for `set-finding` to move, and the block was in a status `restamp` refuses. All three exited 2 before their write while argparse stayed silent, so the sweep counted them as exercised: measured by pointing each register write in turn at the root of the repository, the guard of the promise in `SECURITY.md` stayed green all three times. The stand is now part of the same contract as the argument table, and a refusal no longer counts as a pass. Mutations: each of the three register writes redirected outside `docs/review/` → the sweep red on every one, where it had been green; the other direction — a gate's verdict (exit 1) is work done, not a refusal — is held by its own test, so the rule cannot be tightened into demanding zero from `check` and `coverage`.
- **A CI step counted as running a gate it had been made unable to fail.** The rule compared the declared arguments against the step's and allowed the step to be anything wider. Measured on the real workflow: `|| true` keeps the exit code away from CI, `-k` with a pattern matching nothing runs zero tests, and a commit range of the step's own (`HEAD~1..HEAD`) checks one commit instead of the pull request's — three ways to keep the step and lose the gate, all green. A step may now add only arguments named as widening, and on the place of a declared commit range only the substitution CI takes from the event or the declared range itself. Mutations: the real workflow's test step rewritten each of the three ways → the rule red on each; the other direction — the validator from a venv, the range from the event payload, a louder `-v` — is held by `test_узда_видит_обезвреженные_ворота`.
- **The rule against a paragraph glued to a list item knew one kind of list.** It recognised `-`, `*`, `+` and not a numbered item, while `RELEASING.md` is a numbered procedure end to end: a paragraph written flush under step 4 joins step 4. It also told a continuation from a paragraph by whether the line had any indent, and under `- item` the item's text starts at column 2, so a line indented by one space is swallowed exactly as a flush one is. What separates them is the column the item's text starts at. The list of documents widens with the rule: every tracked `.md` except `docs/review/`, because the role templates are numbered lists of rules an agent works by. Mutations: four shapes the rule missed → red on the old shape; a glued paragraph planted in a role template → red under the new file list, green under the old.
- **The mutation rule for shell gates took its subject from a hand-written list of two scripts.** A fourth gate added tomorrow falls outside it in silence, and `run-role.sh` was already outside — its refusals held by tests, by no rule. The list is checked against `git ls-files` now, and the shape of a refusal widens with it: `if …; then exit 3; fi` and `check || exit 5` are how refusals are ordinarily written and neither was recognised, so `run-role.sh`'s refusal on a lost journal line was invisible too. Mutations: a fourth gate script with two refusals and no test → the rule red naming the script, green before; every refusal of all three scripts silenced one at a time → the named test red on each; the other direction — a whole-line comment explaining an exit code is not a refusal — is in the rule's own table.
- **The rule that a command sweep must check the command started was satisfied by the name of the helper occurring anywhere in it.** A docstring mentioning `argparse_refused`, a dead `argparse_refused = None`, a call whose answer is thrown away, and a sweep calling `self.s.run(*[cmd])` — four ways past it, all measured green. The question is looked for in the syntax now, and the answer must be reached from inside an assertion. Mutations: the four shapes fed to the old rule → invisible, to the new one → each caught; the other direction — a sweep asking through the shared helper and one walking its own copy of the vocabulary — is held by `test_узда_видит_обход_которого_ещё_нет`.
- **Two numbers about the previous round disagreed with what they count.** The CHANGELOG said six findings where the register closes eight, in both languages, with two of the eight appearing in no entry; the round's fix report claimed twenty-three new tests where the suite goes from 325 to 341. Both corrected against the measurement, and the two missing entries written. The class the register records for this — *a number in a public document not checked against its source* — is not closed by a guard for these two: a count of the findings of a ROUND has no source the suite can read, because the register has no notion of a round. That is written down for the maintainer, not papered over.
- **One defect, two records.** The paragraph glued to a list item in both CHANGELOGs was found twice, one round apart, and closed once: `review check` stayed red on the copy that was never moved, and `review findings` offered a block's fixer a finding with nothing behind it. The copy now points at the record that closed it.
- **Two rules for the one glued-paragraph defect, and they disagreed.** Blocks T2 and T4 each fixed the paragraph glued to a list item and each wrote its own rule: T2's exempted a line that opens a block of its own (an HTML block among them) and flagged an indented line never; T4's measured the column the item's text starts at, but exempted a table row and a link reference definition. Neither of the two interrupts a paragraph — CommonMark 4.7 says so of the definition, and GFM builds a table header out of a paragraph's last line — so under a bullet both stay inside it, and a comparison link written there stops being a link. One function now (`_glued_to_list_item`): T4's column, T2's list of block openers (heading, blockquote, thematic break, HTML block), a table row and a link definition flagged; all four tests of both blocks run on it, and the two invented-document tables moved the definition and the row to the glued side.

Merging the T3 refactoring into the review branch, where blocks T2 and T4 and issue #28 had already landed: what they added is carried onto its single entry points, and one guard is new.

- **A merge left its conflict markers in README and AGENTS.md, and nothing in the suite saw them.** git writes `<<<<<<<`, `=======` and `>>>>>>>` into the file as ordinary text, a commit accepts them, and the document then showed both versions of the scenario count at once. `ConflictMarkerTest` reads every file git tracks (binary ones skipped) and refuses the three markers in their order — an opening line, a separating `=======` on its own line (after a diff3 `|||||||` or not), a closing line — naming the file and the line; `=======` alone is a setext heading underline and stays legal. Both sides are held on invented texts. Mutation: the unresolved merge itself (the markers in README.md, README.ru.md and AGENTS.md) → red with every opening line named, and the markers inserted into a temporary copy of the repository → red.
- **What T2, T4 and #28 added now goes through the refactoring's entry points.** The #28 warning about a root where only part of the instances carry a guard is a `Refusals.warn` with its own key, `root/guard-partial`, registered with its test. The three rules those blocks wrote over source (`_named_exits` — parsing once and handing trees to its helpers, `_scope_without_diff`, `_sweeps_without_body_check`) and the new rule below are in `SOURCE_MUTATIONS`, each with a defect and two spellings, instead of being proved only on the code as written; the message rule now reads a message table declared with a type annotation, a spelling it was blind to. T4's extension of the rule-over-rules to rules that read a REPOSITORY FILE (the workflow, a changelog, a script) was keyed on a marker string, `KIT /`, which the refactoring removed; it is a rule of its own now, recognising what the rule is fed — a file read, directly or through a name — and demanding an invented sample beside it. Mutations: the invented samples of `_shell_gates`, `_workflow_steps` and `_glued_to_list_item` removed from the suite → the rule names each; a file rule added with no sample, and the file read hoisted into a variable → red with the same verdict.
- **The sweep gates of #26 speak through the container too.** Merging dev into the review branch: "an enumeration criterion without `sweep`" and "a sweep past `running` without a script" were two lines appending to a list called `problems`, which the refactoring removed — outside the gate registry and outside the mutation run. They are `gates.refuse` with keys of their own, `sweep/undeclared` and `sweep/no-script`, registered with their tests; the tests read the refusal section (`refused`), not the whole output, so a gate turned into a warning goes red as well. The sweep's patterns reach git through `git_files`, the one entry point. The two rules against a paragraph glued to a list item read every tracked document except the review's own reports; the verdict corpus of #23 is those reports verbatim (`tests/corpus/verdicts/finetooth/`), and its frozen parser input is not rewritten to please a markdown rule — both rules now skip one shared list, `VERBATIM_REPORTS`, instead of two spellings of `docs/review/`.

### Added

- **The fixer and the fix reviewer are told the project's commit rules** (issue #33). The fix template told the fixer to commit every fix and never said that the project's own commit rules apply: in the kit's own review, fixers run through `run-role.sh` made twelve commits without `Signed-off-by` under the operator's global git identity, the project's `dco` job refused them, and the PR stood until the history was rewritten. Rule 13 of the fix template, in both languages: before the first commit read the project's commit rules (CONTRIBUTING, AGENTS.md, the CI jobs about commits) and follow them — `git commit -s` where DCO is required, the identity the project requires rather than whatever the machine has. Rule 13 of the fix-review template: run the project's own check of commits over the diff range when there is one (`.github/dco.sh <range>`) and file what it refuses as a finding. The new substitution `{{COMMIT_RULES}}` states what the tool found: a DCO requirement is recognised by `.github/dco.sh`, `.github/dco.yml` (the DCO app) or "DCO", "Signed-off-by", "Developer Certificate of Origin" in a tracked CONTRIBUTING, AGENTS.md or `.github/workflows/*`; the fixer gets the requirement with the files that state it, the fix reviewer gets the ready command over its `--diff` range (the project's script, or `git log` of the trailers when there is none), and a project without such a sign gets a neutral line that names its contribution docs. Looked up at `prompt` time rather than recorded by `setup` — a copy in `blocks.json` would be a second source of truth, stale from the day the project adds DCO. Mutations: detection removed (no sign ever found) → `test_скрипт_dco_даёт_исполнителю_требование_подписи_на_обоих_языках`, `test_ревьюер_правок_прогоняет_проверку_коммитов_проекта_по_диапазону` and `test_требование_в_документах_или_в_ci_узнаётся_без_скрипта` red; only the file-content reading removed → the last one red for CONTRIBUTING, AGENTS.md and the workflow; the project's script not named to the reviewer → the range test red; `{{COMMIT_RULES}}` removed from each of the four templates → red for that role and language; rule 13 removed from a template → `test_правило_стоит_в_шаблонах_и_без_находки_инструмента` red. `test_названный_рядом_с_объёмом_выход_действительно_уменьшает_дифф` measured its "a quarter smaller" on the whole prompt and went red once the template grew by rule 13, though the diff was still halved; the share is now taken on the pasted diff, the whole prompt only has to be smaller.
- **`sweep`: the work an acceptance criterion demands beyond the block's files** (issue #25). A cross-cutting block's criterion is usually an enumeration — "every place that changes data" — and the ceiling counted only `paths`: a live block of 5.7k lines demanded a 47k-line sweep. A block now declares `sweep` (patterns); `sizes` reports it apart ("+ sweep N files / M lines, mechanical, not read"); the hunter's prompt tells it to enumerate by a script at `docs/review/sweeps/<ID>.<ext>` and read the hits; `check` refuses an enumeration criterion without `sweep`, and a block past `running` with a sweep but no script. Mutations: each of the two gates, the sizes line and the criterion detector — red.
...
- `set-finding --clear-rule` removes a recorded guard from the named findings — for a guard measured green on its finding's own defect; `--rule` together with `--clear-rule` is refused. Without it, a wrong guard written before #28 could not be taken back (the kit's self-review, T4-009).
