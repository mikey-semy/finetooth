...
`run-role.sh`, `guard-grep.sh`): 58 findings over the hunt and six rounds of fix review, all confirmed by execution — 56 fixed, 1 deferred and 1 rejected, each with its reason in the register.
...
- **A verdict is a clause, not a word near an id:** the hypothesis id at the start of a line, a dash, the verdict word and a basis. No basis — "checked", "checked: …", the template's `<…>` — is no verdict: the hypothesis stays open and the gate names it. A partial qualifier between the verdict word and the colon ("checked partially:", "проверена лишь частично:") is "not checked"; the word in the basis is not ("checked: a partial index…"). An id mentioned in prose is not an answer; a table row counts only in a table about hypotheses. The old template form (the whole clause in backticks) is still read. Held by `VerdictTableTest` (29 lines both ways), red under each of five mutations; measured on every live register (two projects, 14 blocks): no count moved, one verdict became honest ("проверена частично" → not checked). Rules from the kit author's own review rounds (24.09). One code-span parser instead of two. `import` refuses a draft row over the limits `check` holds. `run-role.sh` exits 3 when the run succeeded but the journal line was lost.
