# Whole-repository review of Toy
...
```sh
python3 <skill>/scripts/review.py status                     # where we are and which block is next
python3 <skill>/scripts/review.py coverage                   # the file → block map; fails if a file is unowned
python3 <skill>/scripts/review.py check                      # the state is consistent
python3 <skill>/scripts/review.py hypotheses H1              # which of the block's hypotheses are closed
python3 <skill>/scripts/review.py prompt H1 --role hunter    # a ready prompt for an agent
```
