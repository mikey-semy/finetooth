...
| T3-026 | T3 | deferred | `tests/test_review.py:5443` | _own_verdict looks only inside cmd_check, so a gate split into a helper that prints its own refusal and exits touches the container nowhere and is invisible to both new guards (fix review round 3, R3-002) |
